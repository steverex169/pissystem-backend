    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            for scheme in schemelist:
                ...
                if scheme.noofanalytes > 0 and scheme.status != 'Active':
                    scheme.status = 'Active'
                    scheme.save()
                elif scheme.noofanalytes == 0 and scheme.status != 'Inactive':
                    scheme.status = 'Inactive'
                    scheme.save()

                scheme_data = model_to_dict(scheme, exclude=['analytes'])  # Exclude non-serializable fields
                analytes = list(scheme.analytes.values('id', 'name', 'code', 'status'))  # Convert analytes to a list of dictionaries
